...
export default function EquipmentSalesForm() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (submissionData.requiredDate) {
                submissionData.requiredDate = submissionData.requiredDate.toISOString();
            }

            const result = await submitEquipmentSalesForm(submissionData);
            console.log('Form submitted successfully:', result);
            setAlertVisible(true);
            setAlertTitle('Success');
            setAlertMessage('Your equipment sales inquiry has been submitted successfully.');
            setFormData({
                name: '',
                email: '',
                businessName: '',
    ...
    const handleDateChange = (event, selectedDate) => {
        const currentDate = selectedDate || formData.requiredDate;
        setShowDatePicker(false);
        setFormData({ ...formData, requiredDate: currentDate });
        setErrors({ ...errors, requiredDate: undefined });
    };
        ...
        <SafeAreaView style={styles.safeArea}>
            <ScrollView style={styles.container}>
                <View style={styles.form}>
                    ...
                    {renderInput('Business Type', 'businessType', 'Enter business type', true)}

                    <View style={styles.inputGroup}>
                        <Text style={styles.label}>Required Date*</Text>
                        <TouchableOpacity onPress={() => setShowDatePicker(true)} style={styles.dateButton}>
                            <Text style={[styles.dateButtonText, errors.requiredDate && styles.inputError]}>
                                {formData.requiredDate ? formData.requiredDate.toDateString() : 'Select Date'}
                            </Text>
                        </TouchableOpacity>
                        {errors.requiredDate && <Text style={styles.errorText}>{errors.requiredDate}</Text>}
                        {showDatePicker && (
                            <DateTimePicker
                                value={formData.requiredDate || new Date()}
                                mode="date"
                                display="default"
